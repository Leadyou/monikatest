...
const STEP_LABELS = { start: "Krok 1 z 3", review: "Krok 2 z 3 — sprawdź dane", result: "Krok 3 z 3 — harmonogram gotowy" };
...
export default function KreatorPage({ onLoginClick }) {
  const [step, setStep] = useState("start"); // start | loading | review | result
    ...
    <div className="app-shell">
      <header className="app-header">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <button className="link-btn" style={{ marginTop: 4 }} onClick={onLoginClick}>
            Zaloguj się
          </button>
        </div>
        <p className="app-subtitle">Zamień wypis ze szpitala na czytelny harmonogram podawania kropli</p>
      </header>

      <main className="page">
        {step !== "loading" && (
          <p style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", color: "var(--accent)", marginBottom: 10 }}>
            {STEP_LABELS[step]}
          </p>
        )}
        ...
        {step === "start" && (
          <>
            <p className="import-intro">
              ...
              kratkami do odhaczania, kolorami nakrętek i miejscem na podpis. Bez zakładania konta.
            </p>
            <label className="import-file-label">
              Wybierz plik PDF z wypisem
              <input type="file" accept="application/pdf" onChange={handleFile} />
            </label>
            <p style={{ fontSize: 13.5, color: "var(--ink-faint)", marginTop: 12, lineHeight: 1.5 }}>
        ...
        {step === "review" && draft && (
          <>
            ...
            {error && <p className="error-text import-error" style={{ marginTop: 14 }}>{error}</p>}
            <button className="btn full" style={{ marginTop: 22, fontSize: 17, padding: "15px 16px" }} onClick={handleShowSchedule}>
              Pokaż harmonogram
            </button>
          </>
        )}
          ...
          <>
            ...
            {phases.map((phase, i) => (
              <div key={i} className="card">
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--ink-faint)" }}>
                  {formatShortDatePL(phase.startDate)} – {formatShortDatePL(phase.endDate)}
                </div>
                <div style={{ fontSize: 16, fontWeight: 700, marginTop: 2 }}>Faza {i + 1}</div>
                <div style={{ fontSize: 14.5, color: "var(--ink-soft)", marginTop: 2 }}>{phase.summary}</div>
              </div>
            ))}

            <button className="btn full" style={{ marginTop: 10, fontSize: 17, padding: "15px 16px" }} onClick={() => window.print()}>
            ...
            <div className="card" style={{ marginTop: 24, borderLeft: "3px solid var(--accent)" }}>
              <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Chcesz, żeby aplikacja pilnowała dawek?</div>
              <p style={{ fontSize: 14.5, color: "var(--ink-soft)", margin: "0 0 12px", lineHeight: 1.5 }}>
